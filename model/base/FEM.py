...
class FEM(nn.Module):
    def __init__(self, inplanes, channel_rate=2, reduction_ratio=16):
        ...
        self.causal_enhancer = nn.Sequential(
            nn.Conv2d(inplanes, inplanes // reduction_ratio, 1),
            nn.ReLU(),
            nn.Conv2d(inplanes // reduction_ratio, inplanes, 1),
            nn.Sigmoid()
        )
    ...
    def forward(self, q, s):
        ...
        E_q = E_q + q
        causal_weights = self.causal_enhancer(E_q)
        E_q = E_q * causal_weights
        return E_q, E_s