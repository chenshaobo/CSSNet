...
class DatasetPASCAL(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, use_original_imgsize):
        ...
        # Add raw transform for SRM (only resize and ToTensor)
    ...
    def __getitem__(self, idx):
        ...
        query_img_raw = self.raw_transform(query_img)  # For SRM post-processing

        if not self.use_original_imgsize:
            query_cmask = F.interpolate(query_cmask.unsqueeze(0).unsqueeze(0).float(),
                                       (400, 400), mode='nearest').squeeze()
        ...
        batch = {'query_img': query_img_transformed,
                 'query_mask': query_mask,
                 'query_name': query_name,
                 'query_ignore_idx': query_ignore_idx,

                 # Add raw image for SRM
                 'query_img_raw': query_img_raw,
